...
class CharacterRepository {
  ...
  Future<(List<Character>, bool /*hasNext*/)> getPage(int page) async {
  ...
  ({List<Character> items, int page, bool hasNext, DateTime? ts}) readCache(Box cache) {
    final raw = (cache.get(_cacheKey) as List?)?.cast<Map>() ?? <Map>[];
    final items = raw.map((e) => Character.fromJson(Map<String, dynamic>.from(e))).toList();
    ...
    return (items: items, page: page, hasNext: hasNext, ts: tsStr == null ? null : DateTime.tryParse(tsStr));
  }
}
